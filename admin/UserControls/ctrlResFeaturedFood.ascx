<%@ Control Language="C#" AutoEventWireup="true" CodeFile="ctrlResFeaturedFood.ascx.cs" Inherits="admin_UserControls_ctrlResFeaturedFood" %>
            ...
            <div class="headingStripConfirm">
                ...
                <div class="floatRight" style="padding-right: 4px;">
                    <asp:LinkButton ID="lbClose" ForeColor="White" Style="text-decoration: none; outline: none"
                        runat="server" Text="[ X ]" OnClientClick="document.getElementById('confirmationBoxBackGround').style.display='none'; document.getElementById('ctl00_ContentPlaceHolder1_ctrlResFeaturedFood2_pnlDelete').style.display='none'; return false;"></asp:LinkButton>
                </div>
            </div>
                        ...
                        <asp:DataGrid ID="gridViewMenus" runat="server" Width="100%" AllowSorting="False"
                            ...
                            <Columns>
                                        ...
                                        <table cellpadding="0" cellspacing="0">
                                            <td style="vertical-align: top" align="left">
                                                &nbsp;&nbsp;<img src='<%# getImagePath(DataBinder.Eval(Container.DataItem,"foodImage")) %>'
                                                    class="menuImageBorder" alt='' width="120px" />
                                                <div>
                                ...
                                <asp:TemplateColumn HeaderText="Food Name">
                                    <ItemTemplate>
                                        <asp:Label ID="lblItemName" CssClass="fontStyle" runat="server" Text='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "foodName").ToString().Length > 22 ? DataBinder.Eval (Container.DataItem, "foodName").ToString().Substring(0,20) + "..." :DataBinder.Eval (Container.DataItem, "foodName").ToString()) %>' ToolTip='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "foodName").ToString()) %>'></asp:Label>
                                    </ItemTemplate>
                                    <EditItemTemplate>
                                        <asp:TextBox ID="txtUpdateFoodName" runat="server" Width="126px" CssClass="txtForm"
                                            MaxLength="200" Text='<%# DataBinder.Eval (Container.DataItem, "foodName") %>'></asp:TextBox>
                                    </EditItemTemplate>
                                </asp:TemplateColumn>
                                <asp:TemplateColumn HeaderText="Description">
                                    <ItemTemplate>
                                        <asp:Label ID="lblDescription" CssClass="fontStyle" runat="server" Text='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "foodDescription").ToString().Length > 45 ? DataBinder.Eval (Container.DataItem, "foodDescription").ToString().Substring(0,41) + "..." :DataBinder.Eval (Container.DataItem, "foodDescription").ToString()) %>' ToolTip='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "foodDescription").ToString()) %>' >
                                        </asp:Label>
                                    </ItemTemplate>
                                    <EditItemTemplate>
                                        <asp:TextBox ID="txtUpdateDescription" runat="server" Width="172px" TextMode="MultiLine"
                                            Height="40px" CssClass="txtForm" MaxLength="1000" Text='<%# DataBinder.Eval (Container.DataItem, "foodDescription") %>'>
                                        </asp:TextBox>
                                    </EditItemTemplate>
                                </asp:TemplateColumn>
                                <asp:TemplateColumn HeaderText="Restaurant Name">
                                    <ItemTemplate>
                                        <asp:Label ID="lblResName" CssClass="fontStyle" runat="server" Text='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "restaurantName").ToString().Length > 25 ? DataBinder.Eval (Container.DataItem, "restaurantName").ToString().Substring(0,25) + "..." :DataBinder.Eval (Container.DataItem, "restaurantName").ToString()) %>' ToolTip='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "foodDescription").ToString()) %>'>
                                        </asp:Label>
                                        <asp:HiddenField ID="hfResID" runat="server" Value='<%# Server.HtmlEncode(DataBinder.Eval (Container.DataItem, "restaurantID").ToString()) %>' />
                                    </ItemTemplate>
                                    <EditItemTemplate>
                                        <asp:DropDownList ID="ddlUpdateSelectRes" Width="172px" runat="server"></asp:DropDownList>
                                    </EditItemTemplate>
                                </asp:TemplateColumn>
